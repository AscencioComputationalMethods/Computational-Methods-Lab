...
class Pendulum:
    def __init__(self, theta_1, theta_2, dt):
    ...
    def polar_to_cartesian(self):
        x1 =  self.length * np.sin(self.theta_1)        
        y1 = -self.length * np.cos(self.theta_1)
        x2 = x1 + self.length * np.sin(self.theta_2)
        y2 = y1 - self.length * np.cos(self.theta_2)
    ...
    def evolve(self):
        ...
        g = self.g
        l = self.length
        expr1 = np.cos(theta_1 - theta_2)
        expr2 = np.sin(theta_1 - theta_2)
        expr3 = (1 + expr2**2)
        expr4 = p1 * p2 * expr2 / expr3
        expr5 = (p1**2 + 2 * p2**2 - p1 * p2 * expr1)         * np.sin(2 * (theta_1 - theta_2)) / 2 / expr3**2
        expr6 = expr4 - expr5
        self.theta_1 += self.dt * (p1 - p2 * expr1) / expr3
        self.theta_2 += self.dt * (2 * p2 - p1 * expr1) / expr3
        self.p1 += self.dt * (-2 * g * l * np.sin(theta_1) - expr6)
        self.p2 += self.dt * (    -g * l * np.sin(theta_2) + expr6) 
...
pendulum = Pendulum(theta_1=np.pi, theta_2=np.pi - 0.01, dt=0.01)
